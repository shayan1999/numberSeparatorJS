...
describe('numberSeparator functions:::', () => {
  it('only number tests::', () => {
    ...
    expect(numberSeparator({text: "223567123", symbol: '-'})).toBe("223-567-123");
    expect(numberSeparator({text: "23477.23"})).toBe("23,477.23");
    expect(numberSeparator({text: "23477.2311", seprateMore: true})).toBe("23,477.2,311");
    expect(numberSeparator({text: "23477.2311", symbol: '-', fixed: 3})).toBe("23-477.231");
  })
  it('find numbers in a full string and put comma::', ()=>{
    for(let i=0;i<examples.length;i++){
      expect(numberSeparator({text: examples[i].testCase})).toBe(examples[i].expect);
    }
  })
  it('exceptions::', ()=>{
    expect(numberSeparator({text: ''})).toBe('');
    expect(numberSeparator({text: '', symbol: '-', fixed: 4, seprateMore: true})).toBe('');
